...
void main(void)
{
	...
	P4DIR &= ~btn;								// Switch input
    P4REN |= btn;                   			// Enable internal pull-up/down resistors
    P4OUT |= btn;                   			// Select a pull-up resistor for the switch
    P4IE |= btn;                       		// Switch interrupt enabled
    P4IES |= btn;                     			// Toggles between high and low mode for the switch
    P4IFG &= ~btn;                  			// Clears the interrupt flag for the switch
    ...
    {
        if(P4IES & btn)							// if both the button interrupt is enabled and the button is held down
        {
        TA0CTL = MC_0;							// Stop the first timer
        TA1CTL = MC_2 + TASSEL_1 + ID_1;		// Start second timer as ACLK divided by 1, in continuous mode
        P4IES &= ~btn;							// Disable/Reenable button interrupt
        P4IFG &= ~btn;							// Clear button interrupt flag
        }
        else if(!(P4IES & btn))				
        {
